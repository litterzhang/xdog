...
class PromptEditorTheme(Protocol):
    """Small styling surface required by :class:`PromptEditor`."""
...
class SlashSelectList:
    ...
    def render(self, width: int, theme: PromptEditorTheme, max_rows: int | None = None) -> list[str]:
...
class PromptEditor(Component):
    ...
    def set_autocomplete_provider(self, provider: CompletionProvider) -> None:
        ...
        PromptEditor owns command-shaped completion; generic completion providers
...
__all__ = ["PromptEditor", "PromptEditorTheme", "SlashSelectList"]
